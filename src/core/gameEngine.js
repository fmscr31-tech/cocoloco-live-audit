...
function syncFromStorage(){const data=loadData();if(data){if(data.players&&data.players.length>0&&gameState.players.length===0){gameState.players.length=0;data.players.forEach(player=>gameState.players.push(player));}gameState.round=getCurrentRound()||data.round||null;gameState.battle=data.battle||null;}gameState.teams=getTeams();setPlayers(gameState.players);setRound(gameState.round);setBattle(gameState.battle);setTeams(gameState.teams);}
...
export function playerWin(id){const canonicalPlayer=getPlayer(id);if(!canonicalPlayer)return null;const canonicalId=canonicalPlayer.id;if(roundWinners.has(canonicalId))return canonicalPlayer;const player=addWin(canonicalId);if(!player)return null;roundWinners.add(player.id);if(getBattle())battlePlayerWin(player.id);let teamSnapshot=null;if(player.teamId){teamSnapshot=addPointsToTeam(player.teamId,1);gameState.teams=getTeams();setTeams(gameState.teams);}setPlayers(gameState.players);createEvent("PLAYER_WIN",{playerId:player.id,name:player.name,points:player.points,wins:player.wins});const winPayload={winId:`win_${Date.now()}_${player.id}`,playerId:player.id,tiktokId:player.tiktokId,id:player.id,name:player.name,username:player.username||player.name,teamId:player.teamId||null,points:player.points,wins:player.wins,wordsFound:player.wordsFound,teamPoints:teamSnapshot?.points??null,timestamp:Date.now()};eventBus.publish("game:score_updated",{playerId:player.id,tiktokId:player.tiktokId,username:player.username||player.name,teamId:player.teamId||null,pointsAdded:1,newTotal:player.points,wins:player.wins,wordsFound:player.wordsFound,teamPointsAdded:player.teamId?1:0,newTeamTotal:teamSnapshot?.points??null,source:"WIN_LIMPIA",timestamp:Date.now(),playerSnapshot:{...player},teamSnapshot:teamSnapshot?{...teamSnapshot}:null});eventBus.emit("win:correct",winPayload);eventBus.emit("overlay:win",{...winPayload,source:"WIN_LIMPIA"});saveState();return player;}
