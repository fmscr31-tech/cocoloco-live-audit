...
function publishPlayersState() {
  try { setPlayers(players); } catch (error) {
...
export function addPlayer(input) {
  ...
  const newPlayer = {
    id: crypto.randomUUID(), name, displayName: name, username, avatar, tiktokId,
    teamId: typeof input === "object" && input !== null ? (input.teamId || null) : null,
    points: 0, wins: 0, wordsFound: 0, roundsPlayed: 0, battlesPlayed: 0, gifts: 0, streak: 0,
    online: true, createdAt: Date.now(), updatedAt: Date.now()
  };
...
/** Reset only the score that belongs to the current round. Identity, team,
 * lifetime wins/words and registration remain intact across GENDER_TEAMS rounds. */
export function resetRoundPlayerScores() {
  let changed = false;
  players.forEach(player => {
    if ((Number(player.points) || 0) !== 0) { player.points = 0; changed = true; }
    if ((Number(player.streak) || 0) !== 0) { player.streak = 0; changed = true; }
    player.updatedAt = Date.now();
  });
  if (changed) { publishPlayersState(); persistPlayers(); }
...
export function addWin(playerId, options = {}) {
  ...
  player.wins++; player.wordsFound++; player.points++; player.streak++; player.updatedAt = Date.now();
  publishPlayersState(); persistPlayers();
  if (options.emitScoreEvent !== false) {
    eventBus.emit("game:score_updated", {
      playerId: player.id, username: player.name, teamId: player.teamId || null,
      pointsAdded: 1, newTotal: player.points, source: "WIN_LIMPIA", timestamp: Date.now(), playerSnapshot: { ...player }
    });
  }
  return player;
}
...
export function addPoints(playerId, points = 1) {
  ...
  player.points = Math.max(0, previousPoints + delta); player.updatedAt = Date.now();
  publishPlayersState(); persistPlayers();
  eventBus.emit("game:score_updated", {
    playerId: player.id, username: player.name, pointsAdded: player.points - previousPoints,
    newTotal: player.points, source: delta < 0 ? "MANUAL_REMOVE" : "POINTS", timestamp: Date.now(), playerSnapshot: { ...player }
  });
  return player;
}
...
export function assignTeam(playerId, teamId) {
  const player = findPlayerById(playerId);
  if (!player) return null;
  player.teamId = teamId || null; player.updatedAt = Date.now();
  publishPlayersState(); persistPlayers();
  eventBus.emit("player:updated", { player: { ...player } });
  return player;
}

export function getPlayer(playerId) { return findPlayerById(playerId); }
export function getPlayers() { return players.map(player => ({ ...player })); }
export function updatePlayer(playerId, patch = {}) {
  const player = findPlayerById(playerId);
  if (!player) return null;
  Object.assign(player, patch, { updatedAt: Date.now() });
  publishPlayersState(); persistPlayers();
  eventBus.emit("player:updated", { player: { ...player } });
  return player;
}
