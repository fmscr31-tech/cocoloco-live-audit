...
import { battleEffectEngine } from "./battleEffectEngine.js";
import { players } from "../playerManager.js";
import { getTeams } from "../TeamManager.js";

class GameRulesEngine {
  constructor() {
    this.rules = { targetPointsToWin: 500, globalMultiplier: 1.0, bonusPerGift: 10 };
  ...
  getPlayerTeam(usernameOrId) {
    ...
    const player = players.find(p => p.id === usernameOrId || String(p.name || "").toLowerCase() === key);
    if (!player || !player.teamId) return null;
    const teams = getTeams();
    return teams.find(t => t.id === player.teamId || String(t.name || "").toLowerCase() === String(player.teamId).toLowerCase()) || null;
  }
  evaluateRewardScoring(reward = {}) {
    ...
    if (battleEffectEngine.isUserFrozen(userId, team ? team.id : null, reward.username)) {
      ...
      eventBus.emit("game:score_redirected", { originalTeam: team ? team.id : null, redirectedTeam: opposingTeam ? opposingTeam.id : null, player: reward.username, points: adjustedPoints, reason: "FREEZE" });
      return;
    }
    eventBus.emit("game:score_updated", { userId: reward.userId, username: reward.username, pointsAdded: adjustedPoints, giftName: reward.giftName, timestamp: Date.now() });
  ...
  getRulesState() { return { rules: { ...this.rules }, battleStatus: { ...this.battleStatus } }; }
}
