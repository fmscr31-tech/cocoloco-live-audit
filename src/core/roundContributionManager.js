...
function beginTrackingFromRound(round){const roundId=round?.id||round?.roundId||null;state={roundId,winLimpia:null,gift:null,contributions:[]};persist();eventBus.publish("mvp:tracking_started",{roundId,timestamp:Date.now()});}
...
export function recordWinLimpia(payload={}){
  ...
  const player=normalizePlayer(payload)||playerSnapshot(payload.playerId); if(!player)return null;
  const contribution={...player,points:1,timestamp:Date.now(),id:payload.winId||`win_${Date.now()}_${Math.random().toString(36).slice(2,8)}`};
  state.winLimpia=contribution; state.contributions.push({type:"WIN_LIMPIA",...contribution});
  recordMvpContribution({player,source:"WIN_LIMPIA",points:1,roundId:state.roundId,contributionId:contribution.id}); persist();
  eventBus.publish("mvp:contribution_pending",{type:"WIN_LIMPIA",player,roundId:state.roundId,contribution:state.winLimpia}); return state.winLimpia;
}
export function recordGift(payload={}){
  const senderId=payload.playerId||payload.tiktokId||payload.id||payload.username;
  const player=normalizePlayer(payload)||playerSnapshot(senderId); if(!player)return null;
  const points=Math.max(1,Number(payload.pointsAdded ?? payload.points ?? 1)||1);
  const contribution={...player,points,giftName:payload.giftName||payload.sourceGift||payload.canonicalGiftId||"Gift",abilityId:payload.abilityId||null,timestamp:Date.now(),id:payload.eventId||payload.giftId||`gift_${Date.now()}_${Math.random().toString(36).slice(2,8)}`};
  state.gift=contribution; state.contributions.push({type:"GIFT",...contribution});
  recordMvpContribution({player,source:"GIFT",points,roundId:state.roundId,contributionId:contribution.id}); persist();
  eventBus.publish("mvp:gift_contribution",{roundId:state.roundId,contribution:state.gift}); return state.gift;
}
export function selectWinLimpiaRecipient(playerId){
  const player=playerSnapshot(playerId); if(!player)return{success:false,reason:"PLAYER_NOT_FOUND"};
  ...
  state.winLimpia=contribution; state.contributions.push({type:"WIN_LIMPIA",...contribution});
  const recorded=recordMvpContribution({player:scored,source:"WIN_LIMPIA",points:1,roundId:state.roundId,contributionId:contribution.id}); persist();
  ...
  eventBus.publish("mvp:contribution_pending",payload); eventBus.publish("mvp:recipient_selected",payload); return{success:true,player:scored,leaderboardEntry:recorded};
}
export function selectGiftRecipient(playerId,giftName="Gift"){
  const player=playerSnapshot(playerId); if(!player)return{success:false,reason:"PLAYER_NOT_FOUND"};
  const contribution={...player,points:1, giftName,selectedManually:true,timestamp:Date.now(),id:`manual_gift_${Date.now()}_${Math.random().toString(36).slice(2,8)}`};
  state.gift=contribution; state.contributions.push({type:"GIFT",...contribution});
  const recorded=recordMvpContribution({player,source:"GIFT",points:1,roundId:state.roundId,contributionId:contribution.id}); persist();
  const payload={type:"GIFT",player,roundId:state.roundId,contribution,leaderboardEntry:recorded||null,timestamp:Date.now()};
  eventBus.publish("mvp:gift_contribution",payload); eventBus.publish("mvp:recipient_selected",payload); return{success:true,player,leaderboardEntry:recorded};
}
