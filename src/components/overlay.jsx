...
// Effective states combining event queue and direct test props
...
const unsubscribe = dashboardAPI.subscribe((dashboard) => {
  updateOverlayFromDashboard(dashboard);
});
...
// Ability Event Queue listener setup for Live Overlay
useEffect(() => {
  const unsubStarted = eventBus.subscribe("ability:started", (item) => {
    console.log("[OVERLAY EVENT RECEIVED]", item);
    console.log("[Overlay] Ability started via queue:", item);
    setShowWin(false);
    setWinner(null);
    ...
    if (item.abilityId === "silent_challenge") {
      ...
      setEpicEvent({
        giftDisplay: `🍩 ${item.sourceGift?.toUpperCase() || "DONUT"} x5`,
      ...
      setEpicEvent({
        giftDisplay: `🤠 ${item.sourceGift?.toUpperCase() || "SOMBRERO"}`,
      ...
      setEpicEvent({
        giftDisplay: `🌌 ${item.sourceGift?.toUpperCase() || "GALAXY"}`,
      ...
      setEpicEvent({
        giftDisplay: `💥 ${item.sourceGift?.toUpperCase() || "MONEY GUN"}`,
  ...
  const unsubFinished = eventBus.subscribe("ability:finished", (item) => {
    console.log("[Overlay] Ability finished via queue:", item);
    if (item.abilityId === "silent_challenge") {
      setDonutTeamId(null);
    } else if (item.abilityId === "creative_challenge") {
      setHatTeamId(null);
    } else if (item.abilityId === "ultimate_galaxy") {
      setGalaxyTeamId(null);
      setGalaxyPopup(null);
    } else if (item.abilityId === "epic_impact") {
      setMoneyGunTeamId(null);
    }
    setEpicEvent(null);
    setAlert(null);
  });
  ...
  const unsubActivated = eventBus.subscribe("effect:activated", (effect) => {
    if (effect.type === "FREEZE") {
      ...
      setFrozenDetails({
        remainingTime: effect.totalDuration || 30,
        activatedBy: effect.activatedBy || "FERNANDO"
      });
    }
  });
  ...
  const unsubUpdated = eventBus.subscribe("effect:updated", (effect) => {
    if (effect.type === "FREEZE") {
      setFrozenDetails(prev => prev ? { ...prev, remainingTime: effect.remainingTime } : { remainingTime: effect.remainingTime, activatedBy: effect.activatedBy });
    }
  });

  const unsubRemoved = eventBus.subscribe("effect:removed", () => {
    setFrozenTeamId(null);
  ...
  const unsubHighlight = eventBus.subscribe("player:highlight", (data) => {
    if (data && data.playerId) {
      setHighlightedPlayerId(data.playerId);
      setTimeout(() => {
...
function updateOverlayFromDashboard(dashboard){
  ...
  if (dashboard.gameMode) {
    setCurrentMode(normalizeMode(dashboard.gameMode));
  }
  ...
  if (dashboard.battleEffects?.active) {
    ...
    setFrozenDetails({
      remainingTime: dashboard.battleEffects.remainingTime || 30,
      activatedBy: dashboard.battleEffects.activatedBy || "FERNANDO"
    });
  }
  ...
  setAlert("🏆 BATALLA FINALIZADA");

  }


  if(event.type==="PLAYER_WIN"){
    console.log("[DEBUG Step 4] Recepción de PLAYER_WIN en Overlay.jsx:", event);
  ...
  setShowWin(true);
  console.log("[DEBUG Step 5] Cambio de estado showWin a true, winner:", event.data);

  setAlert(`👑 ¡CAMPEÓN: ${event.data.name}!`);
...
const oldPlayer=oldState.players.find(
...
setShowWin(false);
  ...
  // Fall back to registered players if round/session is active OR registration has players/is open
...
liveActive: dashboard.liveActive
...
console.log("[TIMER TRACE 07] OVERLAY REACT STATE", updated);
...
{effectiveScare && (
  <div className="scare-popup-banner">
    <div style={{ fontSize: "14px", fontWeight: 900, color: "#ffff00", textTransform: "uppercase", textShadow: "0 0 10px rgba(255,0,0,1)" }}>
      😱 ¡SCARE JUMPSCARE! 😱
    </div>
    <div style={{ fontSize: "9px", fontWeight: 800, color: "#ffffff", marginTop: "2px" }}>
      ¡ALARMA DE SUSTO!
    </div>
  </div>
)}
...
<ScoreBoard
  ...
  donutTeamId={effectiveDonut}
  hatTeamId={effectiveHat}
  galaxyTeamId={effectiveGalaxy}
  moneyGunTeamId={effectiveMoneyGun}
  frozenTeamId={effectiveFrozenTeamId}
  frozenDetails={effectiveFrozenDetails}
  highlightedPlayerId={effectiveHighlightedPlayerId}
  mode={currentMode}
  showWin={effectiveShowWin}
  winner={effectiveWinner}
  liveActive={effectiveLiveActive}
  battleEffects={battleEffects}
/>
...
<GiftFeed
  ...
  showWin={effectiveShowWin}
  winner={effectiveWinner}
  epicEvent={effectiveEpicEvent}
  epicGift={effectiveEpicGift}
/>
