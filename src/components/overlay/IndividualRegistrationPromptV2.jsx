...
const readConfig = () => {
  ...
  return {
    mode: String(config.gameRegistrationMode || "").toUpperCase(),
...
export function IndividualRegistrationPromptV2() {
  ...
  useEffect(() => {
    ...
    const subscriptions = ["registration:opened","registration:closed","registration:updated","registration:state_synced","config:command_updated","GAME_MODE_CHANGED","round:started","round:finished"].map(name => eventBus.subscribe(name, refresh));
    const timer = window.setInterval(refresh, 500);
    return () => { subscriptions.forEach(u => u?.()); window.clearInterval(timer); };
  }, []);

  useEffect(() => setAssetIndex(0), [candidates.join("|")]);
  useEffect(() => { const timer=window.setInterval(()=>setBlink(v=>!v),900); return()=>window.clearInterval(timer); }, []);
  ...
  useEffect(() => {
    const onRoundFinished = payload => {
      ...
      window.setTimeout(() => {
        try {
          const mode = String(dashboardAPI.getGameMode?.() || readConfig().mode || "").toUpperCase();
          if (mode === "INDIVIDUAL") registrationManager.prepareNextRoundRegistration?.();
        } catch {}
        setTick(v => v + 1);
      }, duration + 100);
    };
    const subscriptions = [eventBus.subscribe("round:finished", onRoundFinished), eventBus.subscribe("round:winner_popup", onRoundFinished)];
    return () => subscriptions.forEach(u => u?.());
  }, []);

  const dashboard = dashboardAPI.getLiveDashboard?.() || dashboardAPI.getState?.() || {};
  const dashboardMode = String(dashboard?.gameMode || dashboardAPI.getGameMode?.() || "").toUpperCase();
  const gameMode = dashboardMode || config.mode;
