...
export function TeamPanel({ team, score, players, round, wrapperClass, isFrozen, roundMvpTitle, frozenDetails, isDamaged, isGalaxyBenefited, galaxyPopup, isDonutActive, isCowboyActive }) {
  ...
  const topPlayers = (players || []).filter(p => p.teamId === team.id).sort((a,b) => {
  ...
  useEffect(()=>{
    ...
    topPlayers.forEach((player,index)=>{
      const currentRank=index+1,prevRank=prevRanksRef.current[player.id];
      if(prevRank!==undefined&&prevRank>currentRank){
        const delta=prevRank-currentRank; deltas[player.id]=delta;
    ...
    <div className={`team-wrapper ${wrapperClass}`} style={{position:"relative"}}>
      <div className={`team-card ${isFrozen?"punished":""} ${isDamaged?"damaged":""} ${isDonutActive?"donut-active":""} ${isCowboyActive?"cowboy-active":""} ${isGalaxyBenefited?"galaxy-active":""}`}>
        ...
        <div className="team-name" style={{color:isFrozen?"#fff":isDamaged?"#ffb4b4":isGalaxyBenefited?"#fff":isDonutActive?"#fff":isCowboyActive?"#fff":undefined,textShadow:isFrozen?"0 0 8px rgba(0,245,255,.9)":isDamaged?"0 0 10px rgba(255,0,0,.9)":isGalaxyBenefited?"0 0 10px rgba(125,211,252,.8)":isDonutActive?"0 0 10px rgba(255,105,180,.8)":isCowboyActive?"0 0 12px rgba(255,100,34,.9)":"0 2px 5px rgba(0,0,0,.9)",position:"relative",zIndex:8,minHeight:"18px",display:"flex",alignItems:"center",justifyContent:"center"}}>
          {showCommandPrompt ? <span style={{fontSize:"9px",fontWeight:950,letterSpacing:".3px",textTransform:"uppercase",animation:"commandPromptPulse 1.8s ease-in-out infinite",whiteSpace:"nowrap"}}>📣 ESCRIBE {commandPrompt} PARA UNIRTE</span> : (isFrozen?`❄️ ${team.name} [CONGELADO] ❄️`:isDamaged?`💥 ${team.name} [DESTRUIDO]`:isGalaxyBenefited?`🌌 ${team.name} 🌌`:isDonutActive?`🍩 ${team.name} [EL MUDO]`:isCowboyActive?`🤠 ${team.name} [RETO]`:team.name)}
        </div>
        ...
        <div style={{position:"relative",zIndex:8,width:"100%",display:"flex",justifyContent:"center",alignItems:"center",boxSizing:"border-box"}}>
          {isFrozen?(<div style={{display:"flex",justifyContent:"center",alignItems:"center",width:"100%"}}><div className="freeze-timer-badge"><span style={{fontSize:"11px"}}>❄️ FREEZE:</span><span style={{color:"#fff",fontSize:"12px",marginLeft:"2px"}}>{frozenDetails?.remainingTime||"10"}s</span></div></div>):(<div className={`team-rounds-container ${isGalaxyBenefited?"galaxy-rounds-boost":""}`} style={{position:"relative",overflow:"hidden",display:"flex",justifyContent:"center",alignItems:"center",gap:"3px",minHeight:"18px",padding:"2px 7px",borderRadius:"5px",border:isGalaxyBenefited?"1.5px solid #7dd3fc":isDonutActive?"1.5px solid #ff69b4":isCowboyActive?"1.5px solid #ff8a3d":"1px solid rgba(125,211,252,.55)",background:isGalaxyBenefited?"linear-gradient(135deg,rgba(8,22,38,.98),rgba(24,38,70,.98))":isDonutActive?"linear-gradient(135deg,rgba(38,12,32,.98),rgba(50,18,42,.98))":isCowboyActive?"linear-gradient(135deg,rgba(45,22,10,.98),rgba(60,28,10,.98))":"linear-gradient(135deg,rgba(8,22,38,.98),rgba(22,34,52,.98))",boxShadow:isGalaxyBenefited?"0 0 14px rgba(125,211,252,.55),inset 0 1px 2px rgba(255,255,255,.12)":isDonutActive?"0 0 14px rgba(255,105,180,.5),inset 0 1px 2px rgba(255,255,255,.12)":isCowboyActive?"0 0 14px rgba(255,100,34,.6),inset 0 1px 2px rgba(255,255,255,.12)":"0 2px 8px rgba(0,0,0,.65),inset 0 1px 1px rgba(255,255,255,.12),0 0 10px rgba(125,211,252,.16)"}}><div style={{position:"absolute",inset:0,pointerEvents:"none",background:"linear-gradient(105deg,transparent 0%,transparent 42%,rgba(255,255,255,.28) 49%,transparent 56%,transparent 100%)",backgroundSize:"220% 100%",animation:"roundsShimmer 3.2s ease-in-out infinite"}}/><span className="rounds-title" style={{color:"#fff",fontWeight:900,textShadow:"0 1px 4px rgba(0,0,0,.95)",position:"relative",zIndex:2}}>RONDA</span><span className="rounds-digits" style={{color:"#fff",fontWeight:950,textShadow:"0 1px 5px rgba(0,0,0,.95)",position:"relative",zIndex:2}}>{team.wins||0}</span>{isDamaged&&<span style={{fontSize:"5.5px",color:"#ff8f8f",fontWeight:900,whiteSpace:"nowrap",position:"relative",zIndex:2}}>💥 0 PTS</span>}{isGalaxyBenefited&&<span style={{fontSize:"6px",color:"#dbeafe",fontWeight:900,textShadow:"0 0 4px rgba(125,211,252,.9)",whiteSpace:"nowrap",position:"relative",zIndex:2}}>⚡</span>}{isDonutActive&&<span style={{fontSize:"6px",color:"#ffd1ec",fontWeight:900,textShadow:"0 0 4px rgba(255,105,180,.9)",whiteSpace:"nowrap",position:"relative",zIndex:2}}>🍩</span>}{isCowboyActive&&<span style={{fontSize:"6px",color:"#ffd0ad",fontWeight:900,textShadow:"0 0 6px rgba(255,153,51,1)",whiteSpace:"nowrap",position:"relative",zIndex:2}}>🤠</span>}</div>)}
        </div>
      </div>

      {topPlayers&&topPlayers.length>0&&(<div className="players-box" style={{position:"relative",overflowY:"auto",maxHeight:"220px",border:isFrozen?"1.5px solid #00f0ff":isDamaged?"1px solid rgba(255,51,51,.5)":isGalaxyBenefited?"1px solid #7dd3fc":isDonutActive?"1px solid rgba(255,105,180,.5)":isCowboyActive?"1.5px solid #ff6622":"1px solid rgba(125,211,252,.5)",background:isFrozen?"linear-gradient(145deg,rgba(0,150,200,.4),rgba(0,60,120,.6))":isDamaged?"linear-gradient(145deg,rgba(50,15,15,.92),rgba(20,5,5,.96))":isGalaxyBenefited?"linear-gradient(145deg,rgba(0,30,90,.9),rgba(50,0,140,.9))":isDonutActive?"linear-gradient(145deg,rgba(160,30,90,.85),rgba(90,10,50,.85))":isCowboyActive?"linear-gradient(145deg,rgba(100,40,15,.96),rgba(50,20,10,.96))":"linear-gradient(145deg,rgba(232,238,244,.98),rgba(207,216,226,.98))",boxShadow:isFrozen?"0 0 20px rgba(0,240,255,.6)":isDamaged?"0 0 15px rgba(255,51,51,.4)":isGalaxyBenefited?"0 0 15px rgba(0,210,255,.4)":isDonutActive?"0 0 12px rgba(255,105,180,.3)":isCowboyActive?"0 0 22px rgba(255,100,34,.8)":"0 3px 12px rgba(0,0,0,.5),inset 0 1px 1px rgba(255,255,255,.45)",animation:isGalaxyBenefited?"mvpSlamEntry .3s ease forwards":isCowboyActive?"cowboyPulse 1.5s infinite ease-in-out":undefined}}>
        ...
        {isGalaxyBenefited&&galaxyPopup?(<div style={{padding:"4px 2px",textAlign:"center",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",height:"100%"}}><div style={{fontSize:"11.5px",fontWeight:900,color:"#fff",textShadow:"0 0 8px rgba(125,211,252,.8)",textTransform:"uppercase",letterSpacing:".8px"}}>✨ {galaxyPopup.sender} ✨</div><div style={{fontSize:"7.5px",fontWeight:900,color:"#fff",background:"rgba(0,0,0,.85)",padding:"1.5px 5px",borderRadius:"3px",textTransform:"uppercase",display:"inline-block",boxShadow:"0 0 6px rgba(255,255,255,.35)",letterSpacing:".5px",marginTop:"3px"}}>GALAXY • +1 RONDA</div></div>):(<>
          <div className="players-title" style={{color:"#102a43",position:"relative",zIndex:2,textShadow:"none"}}>{isFrozen?"❄️ MVP CONGELADOS":isDamaged?"💥 BAJO FUEGO":isDonutActive?"🍩 EL MUDO • RETO":isCowboyActive?"🤠 RETO CREATIVO":(roundMvpTitle||"🏆 MVPS (TOP 10)")}</div>
          {topPlayers.map((player,index)=>{const rawPoints=player.points||0;const displayPoints=rawPoints>0?rawPoints:(player.wins||0);const hasRankUp=rankDeltas[player.id]!==undefined;const deltaVal=rankDeltas[player.id];const isWinMvp=String(player.id)===String(winMvpId)||String(player.playerId)===String(winMvpId);const isGiftMvp=String(player.id)===String(giftMvpId)||String(player.playerId)===String(giftMvpId);return <div key={player.id} className={`gamer-player ${hasRankUp?"rank-up-highlight":""}`} style={{position:"relative",zIndex:2,background:hasRankUp?"linear-gradient(135deg,rgba(57,255,136,.28),rgba(255,255,255,.45))":"rgba(255,255,255,.58)",transition:"all .5s ease"}}><div className="player-position">{index===0?"🥇":index===1?"🥈":index===2?"🥉":`${index+1}º`} <span className="player-name" style={{color:"#102a43",fontWeight:900,textShadow:"none"}}>{player.name}</span>{isWinMvp&&<span style={{marginLeft:"4px",fontSize:"8px",fontWeight:950,color:"#15803d"}}>👑 WIN</span>}{isGiftMvp&&<span style={{marginLeft:"4px",fontSize:"8px",fontWeight:950,color:"#a16207"}}>🎁 MVP</span>}{hasRankUp&&<span className="rank-delta-badge" style={{marginLeft:"4px"}}>▲ +{deltaVal}</span>}</div><div className="player-points" style={{color:"#102a43",fontWeight:950,textShadow:"none"}}>{displayPoints} pts</div></div>;})}
        </>)}
      </div>)}
    </div>
  );
