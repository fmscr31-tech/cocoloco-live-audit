  ...
  useEffect(() => {
    ...
    const tick = now => {
      ...
      items.forEach((item, index) => {
        // Slow, independent steering keeps every object alive while making the movement calm.
        const t = now / 1000 + item.phase;
        item.vx += Math.sin(t * (0.31 + index * 0.017)) * 0.00035;
        item.vy += Math.cos(t * (0.27 + index * 0.013)) * 0.00035;
        const speed = Math.hypot(item.vx, item.vy) || 1;
        const targetSpeed = 0.12 + (index % 3) * 0.02;
        ...
        if (item.x <= pad) { item.x = pad; item.vx = Math.abs(item.vx) * 1.01; }
        if (item.x >= maxX) { item.x = maxX; item.vx = -Math.abs(item.vx) * 1.01; }
        if (item.y <= pad) { item.y = pad; item.vy = Math.abs(item.vy) * 1.01; }
        if (item.y >= maxY) { item.y = maxY; item.vy = -Math.abs(item.vy) * 1.01; }
      });
      forceFrame(v => (v + 1) % 2);
...
export function TeamPanel({ team, score, players, round, wrapperClass, isFrozen, roundMvpTitle, frozenDetails, isDamaged, isGalaxyBenefited, galaxyPopup, isDonutActive, isCowboyActive }) {
  ...
  const genderTheme=isGirlsTeam?{
    ...
    border:"rgba(255,106,181,.98)",glow:"rgba(255,91,174,.7)",accent:"#8f174f",pattern:"transparent"
  }:isBoysTeam?{
    background:"rgba(91,201,231,.13)",
    border:"rgba(78,210,255,.98)",glow:"rgba(31,159,232,.72)",accent:"#063d68",pattern:"transparent"
  }:null;
  ...
  const genderDecor=genderTheme?{position:"absolute",inset:0,pointerEvents:"none",zIndex:1,overflow:"hidden",borderRadius:"6px",backgroundImage:"none",backgroundColor:"transparent",mixBlendMode:"normal"}:null;
  ...
  return <div className={`team-wrapper ${wrapperClass} ${isGenderBattle?"gender-team-wrapper":""} ${isGirlsTeam?"gender-girls":""} ${isBoysTeam?"gender-boys":""}`} style={{position:"relative"}}>
    <div className={`team-card ${isFrozen?"punished":""} ${isDamaged?"damaged":""} ${isDonutActive?"donut-active":""} ${isCowboyActive?"cowboy-active":""} ${isGalaxyBenefited?"galaxy-active":""}`} style={genderTheme&&!isFrozen&&!isDamaged&&!isGalaxyBenefited&&!isDonutActive&&!isCowboyActive?{background:genderTheme.background,border:`2px solid ${genderTheme.border}`,boxShadow:`0 0 20px ${genderTheme.glow},inset 0 0 18px rgba(255,255,255,.08),inset 0 -8px 18px rgba(0,0,0,.08)`,animation:"genderCardFloat 4.2s ease-in-out infinite",overflow:"hidden"}:{}}>
