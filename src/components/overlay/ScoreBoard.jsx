...
export function ScoreBoard({ teams, players, timer, round, frozenTeamId, roundMvpTitle, frozenDetails, moneyGunTeamId, galaxyTeamId, galaxyPopup, donutTeamId, hatTeamId, highlightedPlayerId, mode = "team", showWin, winner, onTestWin, liveActive, battleEffects }) {
  ...
  useEffect(() => {
    ...
    const handleRoundWinner = data => {
      ...
      enqueueActivity({
        type: "ROUND_WINNER",
        headerIcon: "🏆",
        headerTitle: "EQUIPO GANADOR",
        donorName: winnerName,
        effectText: "+1 RONDA",
        abilityName: "¡FELICIDADES!"
      });
    };
  ...
  const effectiveWinner = winner || {
    name: mode === "individual" ? (players?.[0]?.name || "FERNANDO") : activeTeams[0]?.name,
    points: mode === "individual" ? (players?.[0]?.points || 0) : activeTeams[0]?.points
  };
        ...
        {currentEvent && (
          <div
            ...
            style={{
              ...
              padding: "5px",
              borderRadius: "7px",
              background: currentEvent.type === "WIN_LIMPIA"
                ? "linear-gradient(135deg,rgba(20,15,40,.98),rgba(140,70,10,.98))"
                : "linear-gradient(135deg,rgba(255,238,140,.98),rgba(255,185,0,.96))",
              border: "1.5px solid #fff",
              boxShadow: currentEvent.type === "WIN_LIMPIA"
                ? "0 0 18px rgba(255,215,0,.7)"
                : "0 0 22px rgba(255,215,0,.75)",
              textAlign: "center"
            }}
          >
            <div style={{ fontSize: "7.5px", fontWeight: 950, color: currentEvent.type === "WIN_LIMPIA" ? "#ffd700" : "#3a2500", textTransform: "uppercase" }}>
              {currentEvent.headerIcon} {currentEvent.headerTitle}
            </div>
            <div style={{ fontSize: "7px", fontWeight: 950, color: currentEvent.type === "WIN_LIMPIA" ? "#fff" : "#0c091a", margin: "2px 0" }}>
              {currentEvent.donorName}
            </div>
            <div style={{ fontSize: "6.5px", fontWeight: 950, color: currentEvent.type === "WIN_LIMPIA" ? "#fff" : "#0c091a" }}>
              {currentEvent.effectText}
            </div>
          </div>
        )}
      </div>
