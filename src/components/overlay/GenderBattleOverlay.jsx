...
export default function GenderBattleOverlay({
  ...
  useEffect(() => {
    ...
    const auxiliaryEvents = ["ability:started", "ability:finished", "cocazo:trigger"];
      ...
      <div className={`gbo-team-column gbo-${side}`}>
        <section className={`gbo-team ${frozen ? "is-frozen" : ""} ${highlighted ? "is-highlighted" : ""} ${abilityActive ? "ability-active" : ""}`}>
          <div className="gbo-team-head">
            <div className="gbo-team-title-wrap"><div className="gbo-team-name">{team.name}</div></div>
          </div>
        ...
        <section className="gbo-mvp-panel" aria-label={`MVP ${team.name}`}>
          ...
          {mvpRows.length === 0 ? <div className="gbo-mvp-empty">ESPERANDO JUGADORES</div> : mvpRows.map((player, index) => {
            const playerName = player.displayName || player.name || player.username || "JUGADOR";
            const medal = index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `${index + 1}º`;
            return <div className="gbo-mvp-row" key={`${player.id || player.playerId || player.username}-${index}`}>
              <span className="gbo-mvp-rank">{medal}</span><span className="gbo-mvp-name" title={playerName}>{playerName}</span><span className="gbo-mvp-points">{Number(player.contributionPoints || 0)} pts</span>
            </div>;
          })}
        </section>
      </div>
  ...
  return (
    <div className="gender-battle-overlay" data-live-active={String(Boolean(liveLiveActive))} data-game-mode={String(liveMode)} data-round={String(liveRound?.number ?? "")}>
      ...
      {showWin && winner && <div className="gbo-win"><div className="gbo-win-card"><div className="gbo-win-kicker">🏆 WIN</div><div className="gbo-win-name">{winner.displayName || winner.name || winner.username || "GANADOR"}</div><div className="gbo-win-points">+{winner.points ?? 1} PUNTO</div></div></div>}
    </div>
  );
}
