    ...
    <div className="timer-container" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      <style>{`
        @keyframes cocoTimerDangerPulse {
          0%,100% { transform:scale(1); filter:drop-shadow(0 0 4px rgba(255,255,255,.9)) drop-shadow(0 0 10px rgba(255,35,35,.9)); }
          50% { transform:scale(1.09); filter:drop-shadow(0 0 6px rgba(255,255,255,1)) drop-shadow(0 0 15px rgba(255,35,35,1)); }
        }
      ...
      `}</style>
      {isIntermission && (
        <div style={{ fontSize:"9px", lineHeight:1, fontWeight:1000, color:"#ffd166", letterSpacing:"1px", textTransform:"uppercase", marginBottom:"3px", animation:"cocoIntermissionPulse 1s ease-in-out infinite", textShadow:"0 0 8px rgba(255,209,102,.75)" }}>
          NUEVA RONDA
        </div>
      )}
      <div
        ...
        style={{
          fontSize: isFiveMinuteAlert ? "35px" : "28px",
          lineHeight: 1,
          fontWeight: "900",
          color: isIntermission ? "#ffd166" : isFiveMinuteAlert ? "#ff3030" : "#ffffff",
          WebkitTextStroke: isIntermission ? "0.4px #3a2100" : isFiveMinuteAlert ? "0.7px #ffffff" : "0px transparent",
          textShadow: isIntermission ? "0 0 8px rgba(255,209,102,.7)" : isFiveMinuteAlert ? "0 0 5px #ffffff, 0 0 12px #ff2222, 0 0 20px rgba(255,70,70,.9)" : "0 0 12px rgba(0,245,255,0.6)",
          transition: "font-size .25s ease, color .2s ease, text-shadow .2s ease",
          animation: isFiveMinuteAlert ? "cocoTimerDangerPulse 1.05s ease-in-out infinite" : isLastMinute ? "cocoTimerSecondsNudge .45s ease-in-out infinite" : "none",
          transformOrigin: "center center",
          whiteSpace: "nowrap"
        }}>
        <span>{formattedMins}:</span>
        <span style={{ color: isLastMinute ? "#ff3030" : "inherit", WebkitTextStroke: isLastMinute ? "0.6px #ffffff" : "inherit", display:"inline-block", animation: isLastMinute ? "cocoTimerSecondsNudge .45s ease-in-out infinite" : "none", textShadow: isLastMinute ? "0 0 4px #fff, 0 0 11px #ff2222" : "inherit" }}>
          {formattedSecs}
        </span>
      </div>
